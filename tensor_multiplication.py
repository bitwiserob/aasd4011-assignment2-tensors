...
def calculate_activation(sum_total):
    ...
    """
    zeros = torch.zeros_like(sum_total)
    res = torch.heaviside(sum_total, zeros)
    return res
...
def calculate_output(X, W, b):
    ...
    """
    mat_prod_bias = calculate_matrix_prod_with_bias(X, W, b)
    res = calculate_activation(mat_prod_bias)
    return res